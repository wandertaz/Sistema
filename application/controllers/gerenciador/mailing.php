...
class mailing extends CI_Controller {
	...
	var $titulo 		= 'Mailing';
	var $dir 			= 'multitools/mailing/';
	var $controller 	= 'multitools/mailing';
	var $title_sing 	= 'CRM';
	var $per_page 		= 20;
	var $table 			= "mailing";
	var $join			= array("maling_selecao_email" => array("where" => " mailing.id = maling_selecao_email.mailing_id", "type" => "inner"),"contato_empresa"=>array("where"=>"contato_empresa.idcontato_empresa =  maling_selecao_email.contato_empresa_idcontato_empresa","type" => "inner"),"inscritos"=>array("where"=>"inscritos.id =  contato_empresa.inscritos_id","type" => "inner"));
        
        
        var $join2			= array("projetos_empresa" => array("where" => " projetos_empresa.inscritos_id = inscritos.id", "type" => "left"),"contato_empresa" => array("where" => " contato_empresa.inscritos_id = inscritos.id", "type" => "inner"));
	...
	public function index(){
		...
		//Registros
		$data['registros'] = $this->default_model->get_all($this->table, array($this->table.'.*'),$offset, $this->per_page, null,null, array($this->table.'.data_envio' => 'DESC', 'titulo' => 'ASC'));
		...
		get_footer(TRUE);
                ...
                public function adicionar_mailing(){
		...
		$data['title_sing'] = $this->title_sing;
                ...
                //Carrega view
                 $this->load->view($this->dir.'form_mailing', $data);                    
		...
		get_footer(TRUE);
                ...
                public function salvar_mailing(){
		//Recebe Post
		$data = $_POST;              
                        
                //Trata os dados
		$data['data'] = w3c_date($_POST['data_envio']);
                $hora =$_POST['hora_envio']?$_POST['hora_envio']:'00:00';
                $data['data_envio'] =$data['data'].' '.$hora;                
                unset($data['data'],$data['hora_envio']);
                
                $data['created']=date('Y-m-d H:m:00');
		...
		redirect($this->controller.'/adicionar_emails/'.$id);
	}
        
        
        
         public function gerenciamento_mailing($id){

		//Cabeçalho
		$titulo = $this->titulo;
		get_header($titulo, TRUE);
		$data['h1'] = 'Gerenciar E-mails';

		//Menu
		get_menu();
                
                //Busca registro dos usuarios com permissão juridica                         
                $data['registros'] = $this->default_model->get_all($this->table, array('contato_empresa.*, maling_selecao_email.created,inscritos.nome as empresa'),null,null, array('mailing_id'=>$id), $this->join, 'nome', 'asc');
		...
		$data['id_mailing'] = $id;
                ...
                //Carrega view
                 $this->load->view($this->dir.'index_emails', $data);                    
        ...
        public function adicionar_emails($id){
		...
		$usuarios = $this->default_model->get_all('usuario', array('*'),null,null, array('tipo'=>'CA','Ativo'=>'S'),null, array('nome' => 'ASC'));
               ...
               //Registros cargos para busca
                $data['cargo'][]='Cargo Contato';
		$cargo = $this->default_model->get_all('contato_empresa', array('distinct(cargo)'),null,null, array('cargo !='=>''),null, array('cargo' => 'ASC'));
                
                for( $x=0;$x<count($cargo);$x++ ) {
                    $data['cargo'][$cargo[$x]->cargo]=$cargo[$x]->cargo;
                    
                }    
                
                
               //Registros area para busca
                $data['area'][]='Área Contato';
		$area = $this->default_model->get_all('contato_empresa', array('distinct(area)'),null,null, array('area !='=>''),null, array('area' => 'ASC'));
                
                for( $x=0;$x<count($area);$x++ ) {
                    $data['area'][$area[$x]->area]=$area[$x]->area;
                    
                } 
                
                
                //Registros cidades para busca
                $data['empresa'][]='Selecionar Empresa';
		$empresa = $this->default_model->get_all('inscritos', array('id , nome'),null,null, array('tipo_pessoa'=>'J','ativo'=>'S'),null, array('nome' => 'ASC'));
                
                for( $x=0;$x<count($empresa);$x++ ) {
                    $data['empresa'][$empresa[$x]->id]=$empresa[$x]->nome;                    
                }  
                ...
                //Carrega view
                 $this->load->view($this->dir.'form_emails', $data);                    
		...
		get_menu();
                ...
                if($this->input->get('tipo_filtro')=='E'){    // busca filtro empresas                
                
                    
                     if ($this->input->get('nome')!=''){
                        $data_busca['inscritos.nome']= $this->input->get('nome');                        
                    }
                    
                    
                    
                    if($this->input->get('atuacao_empresa')!='')   
                        $data['where']['inscritos.atuacao_empresa'] = $this->input->get('atuacao_empresa');

                     if($this->input->get('segmento_empresa')!='')    
                        $data['where']['inscritos.segmento_empresa'] = $this->input->get('segmento_empresa');                       

                    if($this->input->get('cidade')!='' && $this->input->get('cidade')>0 ) 
                       $data['where']['inscritos.cidade'] = $this->input->get('cidade');                      


                    if($this->input->get('categoria')!=''){
                        if($this->input->get('categoria')=='C'){                          
                           $data['where']['inscritos.cliente'] = 'S';
                        }
                       if($this->input->get('categoria')=='P'){
                            $data['where']['inscritos.prospect'] = 'S';                          
                        }
                    }

                    $join=array("contato_empresa" => array("where" => " contato_empresa.inscritos_id = inscritos.id", "type" => " inner"));
                    if($this->input->get('tipo')!=''){
                        $join=  $this->join2;
                        $data['where']['projetos_empresa.tipo'] = $this->input->get('tipo');
                        //$data_busca['projetos_empresa.tipo'] = $this->input->get('tipo');                    
                    }  

                    if($this->input->get('id_usuario_consultor_responsavel')!='' && $this->input->get('id_usuario_consultor_responsavel')>0){
                        $join=  $this->join2;
                        //$data_busca['projetos_empresa.id_usuario_consultor_responsavel'] = $this->input->get('id_usuario_consultor_responsavel');
                        $data['where']['projetos_empresa.id_usuario_consultor_responsavel'] = $this->input->get('id_usuario_consultor_responsavel');
                    }   
                    if($this->input->get('classificacao')!='')     
                         $data['where']['inscritos.classificacao'] = $this->input->get('classificacao');
                    ...
                    if (!isset($data_busca)){
                        ...
                        $data['registros'] = $this->default_model->get_by_search('inscritos', array('contato_empresa.*','inscritos.razao_social'), $data['where'], null, null, $data_busca,$join, 'inscritos.id', 'asc');
		
                                                      
                }elseif($this->input->get('tipo_filtro')=='C'){ // busca filtro contatos
                    
                   
                    if($this->input->get('sexo')!='')                   
                        $data_busca[ 'contato_empresa.sexo'] = $this->input->get('sexo');


                     if($this->input->get('contato_principal')!='')                   
                        $data_busca['contato_empresa.contato_principal'] = $this->input->get('contato_principal');

                    if($this->input->get('cargo')!='' && $this->input->get('cargo')>0 )                    
                       $data_busca['contato_empresa.cargo'] = $this->input->get('cargo');
                    
                    if($this->input->get('area')!='' && $this->input->get('area')>0 )                    
                       $data_busca['contato_empresa.area'] = $this->input->get('area');
                    ...
                    if($this->input->get('data_nascimento')!=''){          
                        $mes = substr($this->input->get('data_nascimento'),3,2);
                        $dia = substr($this->input->get('data_nascimento'),0,2);                
                        $data_pesquisa= date('Y').'-'.$mes."-".$dia;   
                        
                        $data['where']['vw_aniversario.data_busca'] = $data_pesquisa;
                         $data_busca['contato_empresa.inscritos_id']  = '';
                    }
                    
                     if($this->input->get('inscritos_id')!='' && $this->input->get('inscritos_id')>0 ) {
                       $data['where']['contato_empresa.inscritos_id'] = $this->input->get('inscritos_id');
                       $data_busca['contato_empresa.inscritos_id']  = '';
                       
                     }                       
                    
                     if(!isset($data['where']))
                        $data['where']['contato_empresa.email !='] ='';
                     
                    if (!isset($data_busca))
                        $data_busca['contato_empresa.idcontato_empresa']  = '0';
                    
                    
                    $join=array("inscritos" => array("where" => " contato_empresa.inscritos_id = inscritos.id", "type" => " inner"),"vw_aniversario" => array("where" => " contato_empresa.idcontato_empresa = vw_aniversario.idcontato_empresa", "type" => "inner"));
                    
                    $data['registros'] = $this->default_model->get_by_search('contato_empresa', array('contato_empresa.*','inscritos.razao_social'), $data['where'], null, null, $data_busca,$join, 'contato_empresa.nome', 'asc');
		...
		$data['id_mailing']=  $this->input->get('id_mailing');           
		...
		$dat = $_POST;
                $data['created']=date('Y-m-d H:m:00');
                $data['mailing_id']= $dat['id_mailing'];
                              
		//Salva so dados (Verificando se é edição ou inserção)
		$rows_affected=0;
                foreach ($dat['contato_empresa_idcontato_empresa'] as $itens) {
                        $data['contato_empresa_idcontato_empresa']=$itens;
                        
                        $busca_email = $this->default_model->get_all('maling_selecao_email', array('*'),null,null, array('contato_empresa_idcontato_empresa'=>$itens,'mailing_id'=>$data['mailing_id']),null, array('id' => 'ASC'));
                        if(count($busca_email)==0){
                           $rows_affected = $this->default_model->insert('maling_selecao_email', $data);
                           //$rows_affected=1;
                        }
		...
		if($rows_affected == 1){
                   $config= array(
                    ...
                    $mailing = $this->default_model->get_all('mailing', array('*'),null,null, array('id'=>$data['mailing_id']),null, array('id' => 'ASC'));
                    ...
                    $type = 'regular';
                     ...
                     //campo obrigatório opçoes de configuração
                    $options['list_id']=$this->listId;
                    $options['subject']=$mailing[0]->titulo;
                    $options['from_email']='vinicius@multiwebdigital.com.br';
                    $options['from_name']='MB Consultoria';
                    //$options['to_name']='wander';
                    
                    $options['authenticate'] = TRUE;                    
                    $options['tracking']=array('opens' => true, 'html_clicks' => true, 'text_clicks' => false);
                    
                    //monta o html da campanha
                    $content = array('html'=>$mailing[0]->texto);
                    
                    // aqui pegamos todos os email para criacao da campanha
                    $join=array("contato_empresa" => array("where" => " contato_empresa.idcontato_empresa = maling_selecao_email.contato_empresa_idcontato_empresa", "type" => "inner"));
                    $email_campanha = $this->default_model->get_all('maling_selecao_email', array('nome , email'),null,null, array('maling_selecao_email.mailing_id'=>$data['mailing_id']),$join, array('nome' => 'ASC'));
                    foreach ($email_campanha as $emails) {
                        $segs[] = array('field'=>'EMAIL', 'op'=>'eq', 'value'=>$emails->email);
                         
                    }
                   
                    // seleciona os emails que estão dentro de uma  lista para envio
                   $segmento['match'] = 'any';
                   $segmento['conditions'] = $segs;
                    
                   // cria a campanha
                   $campaignId = $this->mail_chimp->campaignCreate($type, $options, $content,$segmento);
                    
                    
                  
                    // usado para debugar imprimir o erro do mailchimp
                     //$and=$this->mail_chimp->errorMessage;
                   
                    if( $mailing[0]->data_envio=='' || br_date($mailing[0]->data_envio) <= date('d/m/Y')){ // aqui ele envia a campanha agora
                        $retval =  $this->mail_chimp->campaignSendNow($campaignId);
                        $and='campanha enviada.';
                        
                    }else{// aqui ele  agenda o envio para a data hora preenchida
                        $schedule_for = $mailing[0]->data_envio;
                        $retval = $this->mail_chimp->campaignSchedule($campaignId, $schedule_for);
                        
                        $and='agendamento da campanha marcado.';
                    }
                    
                    
			$msg = 'Dados salvos com sucesso, '.$and;
                }else{
			$msg = 'Não foi possível salvar os dados.';
                }
		//Retorno
		...
		redirect($this->controller.'/gerenciamento_mailing/'.$data['mailing_id']);
	} 
		...
		//Busca registro
		$data['registro'] = $this->default_model->get_by_id('proposta', $id);
	...
	public function buscar(){
		...
		//Parâmetros de busca
             ...
             if (!isset($data_busca))
               $data_busca[$this->table.'.nome']  = '';
                ...
                $offset = $this->input->get('per_page');              
		$data['registros'] = $this->default_model->get_by_search($this->table, array($this->table.'.*'), $data['where'], $offset, $this->per_page, $data_busca,$join, $this->table.'.id', 'DESC');
        ...
        public function enviar_lembrete() {
            ...
            $query = "  select *,cast(ADDDATE(now(), 7) as date) AS COMPARACAO";
