...
class Posts extends CI_Controller {
	...
	var $titulo 		= 'Posts';
	var $dir 			= 'multitools/posts/';
	var $controller 	= 'multitools/posts';
	var $title_sing 	= 'Post';
	var $per_page 		= 20;
	var $table 			= 'posts';
	var $join			= array('idiomas' => array('where' => 'idiomas.id = idioma_id', 'type' => 'inner'),
								'posts_categorias' => array('where' => 'posts_categorias.id = categoria_id', 'type' => 'inner'),
	...
	public function index($offset = 0){
		...
		//Registros
		$data['registros'] = $this->default_model->get_all($this->table, array($this->table.'.*, idiomas.nome as idioma, posts_categorias.categoria as categoria, posts_colunistas.nome as colunista'), $offset, $this->per_page, array(), $this->join, $this->table.'.id', 'DESC');
	...
	public function adicionar(){
		...
		//Busca Idiomas, Categorias e Colunistas
		$data['idiomas'] = $this->default_model->listaAssociativa('idiomas', 'nome');
		$data['categorias'] = $this->default_model->listaAssociativa('posts_categorias', 'categoria');
		$data['colunistas'] = $this->default_model->listaAssociativa('posts_colunistas', 'nome');
	...
	public function buscar(){
		...
		$data['registros'] = $this->default_model->get_by_search($this->table, array($this->table.'.*, idiomas.nome as idioma, posts_categorias.categoria as categoria, posts_colunistas.nome as colunista'), NULL, $offset, $this->per_page, $data_busca, $this->join, $this->table.'.id', 'ASC');
