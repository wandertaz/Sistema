...
class Idiomas_selecao extends CI_Controller {

	var $titulo 		= 'Idiomas';
	var $dir 			= 'multitools/idiomas_selecao/';
	var $controller 	= 'multitools/idiomas_selecao';
	var $title_sing 	= 'Idioma';
	var $per_page 		= 20;
	var $table 			= 'idiomas_selecao';
	var $join			= NULL;
	...
	public function editar($id){
		...
		//Busca registro
		$data['registro'] = $this->default_model->get_by_id($this->table, $id, array('*'), NULL, NULL, 'id_idioma');
	...
	public function salvar()
	{
		...
		if(isset($data["id_idioma"]) && $data["id_idioma"])
			$rows_affected = $this->default_model->update($this->table, $_POST['id_idioma'], $data, 'id_idioma');
		else{
	...
	public function excluir($id){
		...
		//Exclui registro
		if($this->default_model->update($this->table, $id, array('ativo' => 'N'), 'id_idioma'))
	...
	public function buscar(){
		...
		//Parâmetros de busca
		$data_busca[$this->table.'.nome_idioma']  = $this->input->get('s');
