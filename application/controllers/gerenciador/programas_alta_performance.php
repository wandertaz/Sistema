...
class Programas_alta_performance extends CI_Controller {

	var $titulo 		= 'Programa Alta Performance';
	var $dir 			= 'multitools/programas_alta_performance/';
	var $controller 	= 'multitools/programas_alta_performance';
	var $title_sing 	= 'Programa Alta Performance';
	var $per_page 		= 20;
	var $table 			= 'programas_alta_performance';
	...
	public function salvar()
	{
		...
		$data = $_POST;
                ...
                // trata  as datas de iíncio e conclusão
                $data['data_inicio'] = str_replace('/','-',$data['data_inicio']);
                $data['data_inicio']= date('Y-m-d', strtotime($data['data_inicio']));

                $data['data_conclusao'] = str_replace('/','-',$data['data_conclusao']);
                $data['data_conclusao']= date('Y-m-d', strtotime($data['data_conclusao']));

		//Trata os dados
		$data['data_limite'] = w3c_date($_POST['data_limite']);
		$data['valor'] = numero_pt_para_mysql($data['valor']);
		$data['valor_juridica'] = numero_pt_para_mysql($data['valor_juridica']);
