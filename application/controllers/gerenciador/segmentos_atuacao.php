...
class Segmentos_atuacao extends CI_Controller {

	var $titulo 		= 'Segmentos de Atuação';
	var $dir 			= 'multitools/segmentos_atuacao/';
	var $controller 	= 'multitools/segmentos_atuacao';
	var $title_sing 	= 'Segmento de Atuação';
	var $per_page 		= 20;
	var $table 			= 'segmentodeatuacao';
	var $join			= NULL;
	...
	public function editar($id){
		...
		//Busca registro
		$data['registro'] = $this->default_model->get_by_id($this->table, $id, array('*'), NULL, NULL, 'segmentodeatuacao_id');
	...
	public function salvar()
	{
		...
		if(isset($data["segmentodeatuacao_id"]) && $data["segmentodeatuacao_id"])
			$rows_affected = $this->default_model->update($this->table, $_POST['segmentodeatuacao_id'], $data, 'segmentodeatuacao_id');
		else{
	...
	public function excluir($id){
		...
		//Exclui registro
		if($this->default_model->update($this->table, $id, array('ativo' => 'N'), 'segmentodeatuacao_id'))
	...
	public function buscar(){
		...
		//Parâmetros de busca
		$data_busca[$this->table.'.segmentodeatuacao_nome']  = $this->input->get('s');
