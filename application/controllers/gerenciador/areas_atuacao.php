...
class Areas_atuacao extends CI_Controller {
	...
	var $titulo 		= 'Áreas de Atuação';
	var $dir 			= 'multitools/areas_atuacao/';
	var $controller 	= 'multitools/areas_atuacao';
	var $title_sing 	= 'Área de Atuação';
	var $per_page 		= 20;
	var $table 			= 'area_de_atuacao';
	var $join			= NULL;
	...
	public function editar($id){
		...
		//Busca registro
		$data['registro'] = $this->default_model->get_by_id($this->table, $id, array('*'), NULL, NULL, 'id_area');
	...
	public function salvar()
	{
		...
		if(isset($data["id_area"]) && $data["id_area"])
			$rows_affected = $this->default_model->update($this->table, $_POST['id_area'], $data, 'id_area');
		else{
	...
	public function excluir($id){
		...
		//Exclui registro
		if($this->default_model->update($this->table, $id, array('ativo' => 'N'), 'id_area'))
