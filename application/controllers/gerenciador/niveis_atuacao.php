...
class Niveis_atuacao extends CI_Controller {

	var $titulo 		= 'Níveis de Atuação';
	var $dir 			= 'multitools/niveis_atuacao/';
	var $controller 	= 'multitools/niveis_atuacao';
	var $title_sing 	= 'Nível de Atuação';
	var $per_page 		= 20;
	var $table 			= 'niveis_de_atuacao';
	var $join			= NULL;
	...
	public function editar($id){
		...
		//Busca registro
		$data['registro'] = $this->default_model->get_by_id($this->table, $id, array('*'), NULL, NULL, 'id_nivel');
	...
	public function salvar()
	{
		...
		if(isset($data["id_nivel"]) && $data["id_nivel"])
			$rows_affected = $this->default_model->update($this->table, $_POST['id_nivel'], $data, 'id_nivel');
		else{
	...
	public function excluir($id){
		...
		//Exclui registro
		if($this->default_model->update($this->table, $id, array('ativo' => 'N'), 'id_nivel'))
	...
	public function buscar(){
		...
		//Parâmetros de busca
		$data_busca[$this->table.'.nome_nivel']  = $this->input->get('s');
